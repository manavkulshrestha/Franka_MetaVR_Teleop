...
def build_request(*, wrist_rgb: np.ndarray, left_rgb: np.ndarray, right_rgb: np.ndarray|None,
    ...
    if right_rgb is not None:
        req['observation/exterior_image_2_right'] = image_tools.resize_with_pad(right_rgb, 224, 224)
    return req
...
def main(ctrl_period: float = 1/20, angled45: bool = True, *,
    ...
    cam_sns = [wrist_cam_sn, left_cam_sn]
    if right_cam_sn is not None:
        cam_sns.append(right_cam_sn)
    cams = [FemtoMega(sn) for sn in cam_sns]
    ...
    try:
        while True:
            if action_chunk is None or action_index >= min(open_loop_horizon, len(action_chunk)):
                wrist_rgb, left_rgb, *right_rgb_ptr = [cam.get_rgb() for cam in cams]
                request = build_request(
                    wrist_rgb=wrist_rgb,
                    left_rgb=left_rgb,
                    right_rgb=right_rgb_ptr[0] if right_rgb_ptr else None,
